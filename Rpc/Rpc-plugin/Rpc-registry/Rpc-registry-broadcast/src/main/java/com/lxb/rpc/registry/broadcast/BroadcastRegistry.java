...
import com.lxb.rpc.util.network.Ipv4;
        ...
        protected CompletableFuture<Void> doSubscribe(final ClusterBooking booking) {
            return Futures.call(new Futures.Executor<Void>() {
                ...
                public void execute(final CompletableFuture<Void> future) {
                    ...
                    map.values().forEach(url -> events.add(new ClusterEvent.ShardEvent(new Shard.DefaultShard(url), ClusterEvent.ShardEventType.ADD)));
    ...
    protected static class BroadcastClusterBooking extends ClusterBooking implements EntryAddedListener<String, URL>,
        ...
        public void entryAdded(final EntryEvent<String, URL> event) {
            handle(new ClusterEvent(this, null, UPDATE, stat.incrementAndGet(),
                    Collections.singletonList(new ClusterEvent.ShardEvent(new Shard.DefaultShard(event.getValue()), ClusterEvent.ShardEventType.ADD))));
        }

        @Override
        public void entryExpired(final EntryEvent<String, URL> event) {
            handle(new ClusterEvent(this, null, UPDATE, stat.incrementAndGet(),
                    Collections.singletonList(new ClusterEvent.ShardEvent(new Shard.DefaultShard(event.getOldValue()), ClusterEvent.ShardEventType.DELETE))));
        }
        ...
        public void entryUpdated(final EntryEvent<String, URL> event) {
            handle(new ClusterEvent(this, null, UPDATE, stat.incrementAndGet(),
                    Collections.singletonList(new ClusterEvent.ShardEvent(new Shard.DefaultShard(event.getValue()), ClusterEvent.ShardEventType.UPDATE))));
        }
