...
final class SnappyCompressor {
    ...
    private static final int COPY_1_BYTE_OFFSET = 1;  // 3 bit length + 3 bits of offset in opcode
    ...
    private static int[] findCandidate(byte[] input, int ipIndex, int ipLimit, int inputOffset, int shift,
        ...
        for (ipIndex += 1; ipIndex + bytesBetweenHashLookups(skip) <= ipLimit; ipIndex += bytesBetweenHashLookups(skip++)) {
            ...
            int currentInt = Memory.getInt(input, ipIndex);
            ...
            if (currentInt == Memory.getInt(input, candidateIndex)) {
                break;
            }
        }
    ...
    private static int[] emitCopies(byte[] input, final int inputOffset, final int inputSize, int ipIndex,
        ...
        do {
            ...
            assert equals(input, ipIndex, input, candidateIndex, matched);
            ipIndex += matched;
            ...
            if (Memory.FAST_ACCESS_SUPPORTED) {
                long foo = Memory.getLong(input, ipIndex - 1);
                prevInt = (int) foo;
                inputBytes = (int) (foo >>> 8);
            } else {
                prevInt = Memory.getInt(input, ipIndex - 1);
                inputBytes = Memory.getInt(input, ipIndex);
            }
        ...
        } while (inputBytes == Memory.getInt(input, candidateIndex));
        return new int[]{ipIndex, outputIndex};
    }
    ...
    private static int emitLiteral(byte[] output, int outputIndex, byte[] literal, final int literalIndex,
        ...
        if (n < 60) {
            ...
            if (allowFastPath && length <= 16) {
                Memory.copyLong(literal, literalIndex, output, outputIndex);
                Memory.copyLong(literal, literalIndex + 8, output, outputIndex + 8);
                outputIndex += length;
                return outputIndex;
            }
        } else if (n < (1 << 8)) {
    ...
    private static int findMatchLength(byte[] s1, int s1Index, byte[] s2, final int s2Index, int s2Limit) {
        ...
        if (Memory.FAST_ACCESS_SUPPORTED) {
            ...
            while (s2Index + matched <= s2Limit - 4 && Memory.getInt(s2, s2Index + matched) == Memory
                    .getInt(s1, s1Index + matched)) {
                matched += 4;
            }

            if (NATIVE_LITTLE_ENDIAN && s2Index + matched <= s2Limit - 4) {
                int x = Memory.getInt(s2, s2Index + matched) ^ Memory.getInt(s1, s1Index + matched);
