...
import com.lxb.rpc.util.Memory;

final class SnappyDecompressor {
    private static final int MAX_INCREMENT_COPY_OVERFLOW = 20;
    private static final int LITERAL = 0;

    public static int getUncompressedLength(final byte[] compressed, final int compressedOffset) throws
            CorruptionException {
        return readUncompressedLength(compressed, compressedOffset)[0];
    }

    public static byte[] uncompress(final byte[] compressed, int compressedOffset, int compressedSize) throws
            CorruptionException {
    ...
    public static int uncompress(final byte[] compressed, int compressedOffset, int compressedSize,
                                 final byte[] uncompressed, int uncompressedOffset) throws CorruptionException {
        // Read the uncompressed length from the front of the compressed input
        int[] varInt = readUncompressedLength(compressed, compressedOffset);
        int expectedLength = varInt[0];
        compressedOffset += varInt[1];
        ...
        if (expectedLength > uncompressed.length - uncompressedOffset) {
            throw new IllegalArgumentException(String.format("Uncompressed length %d must be less than %d",
                    expectedLength, uncompressed.length - uncompressedOffset));
        }

        // Process the entire input
        int uncompressedSize =
                decompressAllTags(compressed, compressedOffset, compressedSize, uncompressed, uncompressedOffset);
    ...
    private static int decompressAllTags(final byte[] input, final int inputOffset, final int inputSize,
        ...
        while (ipIndex < ipLimit - 5) {
            int opCode = Memory.getByte(input, ipIndex++);
            int entry = Memory.getShort(opLookupTable, opCode);
            ...
            if ((opCode & 0x3) == LITERAL) {
                ...
                // position 1. Thus, ten excess bytes.
                {
                    ...
                    if (length <= 16 && copyOffset >= 8 && spaceLeft >= 16) {
                        ...
                        Memory.copyLong(output, srcIndex, output, opIndex);
                        Memory.copyLong(output, srcIndex + 8, output, opIndex + 8);
     ...
     */
    private static int[] decompressTagSlow(final byte[] input, int ipIndex, final byte[] output, int outputLimit,
                                           int outputOffset, int opIndex) throws CorruptionException {
        // read the op code
        int opCode = Memory.getByte(input, ipIndex++);
        int entry = Memory.getShort(opLookupTable, opCode);
        int trailerBytes = entry >>> 11;
    ...
    private static int readTrailer(final byte[] data, final int index, final int bytes) {
        return Memory.getInt(data, index) & wordmask[bytes];
    }

    private static void copyLiteral(final byte[] input, final int ipIndex, final byte[] output, final int opIndex,
                                    final int length) throws CorruptionException {
        assert length > 0;
        assert ipIndex >= 0;
        assert opIndex >= 0;
        ...
        if (length <= 16 && spaceLeft >= 16 && readableBytes >= 16) {
            Memory.copyLong(input, ipIndex, output, opIndex);
            Memory.copyLong(input, ipIndex + 8, output, opIndex + 8);
        } else {
            int fastLength = length & 0xFFFFFFF8;
            if (fastLength <= 64) {
                // copy long-by-long
                for (int i = 0; i < fastLength; i += 8) {
                    Memory.copyLong(input, ipIndex + i, output, opIndex + i);
                }
                ...
                Memory.copyMemory(input, ipIndex, output, opIndex, length);
            }
        }
    }
     ...
     * or memmove().
     */
    private static void incrementalCopy(final byte[] src, int srcIndex, final byte[] op, int opIndex, int length) {
        do {
            op[opIndex++] = src[srcIndex++];
        } while (--length > 0);
    }

    private static void incrementalCopyFastPath(final byte[] output, int srcIndex, int opIndex, int length) {
        int copiedLength = 0;
        while ((opIndex + copiedLength) - srcIndex < 8) {
            Memory.copyLong(output, srcIndex, output, opIndex + copiedLength);
            copiedLength += (opIndex + copiedLength) - srcIndex;
        }

        for (int i = 0; i < length - copiedLength; i += 8) {
            Memory.copyLong(output, srcIndex + i, output, opIndex + copiedLength + i);
        }
    }
    ...
    private static final short[] opLookupTable =
            new short[]{0x0001, 0x0804, 0x1001, 0x2001, 0x0002, 0x0805, 0x1002, 0x2002, 0x0003, 0x0806, 0x1003, 0x2003,
     ...
     */
    private static int[] readUncompressedLength(final byte[] compressed, int compressedOffset) throws
            CorruptionException {
        int result;
        int bytesRead = 0;
        {
