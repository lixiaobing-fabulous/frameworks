...
import com.lxb.rpc.exception.ProtocolException;
import com.lxb.rpc.util.Maps;
import com.lxb.rpc.util.Switcher;
...
public class AbstractRegistar implements Registar {
        ...
        return switcher.reader().quietAnyway(() -> {
            ...
            if (publisher.addHandler(handler)) {
                ...
                if (shards != null && !shards.isEmpty()) {
                    List<ClusterEvent.ShardEvent> events = new ArrayList<>(shards.size());
                    shards.forEach(o -> events.add(new ClusterEvent.ShardEvent(o, ADD)));
                    publisher.offer(new ClusterEvent(this, handler, UpdateEvent.UpdateType.FULL, meta.updates, events));
                }
                return true;
            }
            return false;
    ...
    protected boolean compare(final String name, final long updates, final List<Shard> olds, final List<Shard> targets) {
        List<ClusterEvent.ShardEvent> events = new LinkedList<>();
        ClusterEvent                  event  = new ClusterEvent(this, null, UpdateEvent.UpdateType.UPDATE, updates, events);
        if (olds == null || olds.isEmpty()) {
            targets.forEach(o -> events.add(new ClusterEvent.ShardEvent(o, ClusterEvent.ShardEventType.ADD)));
        } else {
            //原有分片键值对
            ...
            for (Shard shard : targets) {
                ...
                if (old == null) {
                    //新增的
                    events.add(new ClusterEvent.ShardEvent(shard, ClusterEvent.ShardEventType.ADD));
                } else if (!old.equals(shard)) {
                    //修改的
                    events.add(new ClusterEvent.ShardEvent(shard, ClusterEvent.ShardEventType.UPDATE));
                }
            }
            //剩下是删除的
            for (Shard shard : map.values()) {
                events.add(new ClusterEvent.ShardEvent(shard, ClusterEvent.ShardEventType.DELETE));
            }
        }
        //广播通知
        if (!events.isEmpty()) {
