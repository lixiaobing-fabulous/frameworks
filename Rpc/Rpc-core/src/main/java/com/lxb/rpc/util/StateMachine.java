...
import static com.lxb.rpc.util.StateTransition.*;
...
public class StateMachine<T, S extends StateTransition, M extends StateController<T>> {
    ...
    public static class ExStateMachine<T, M extends StateController.ExStateController<T>> extends StateMachine<T, StateInt, M> {
        ...
        public ExStateMachine(final Supplier<M> controllerSupplier,
                              final Function<String, Throwable> errorFunc,
                              final StateFuture.ExStateFuture<T> stateFuture) {
        ...
        public ExStateMachine(final String name,
                              ...
                              final StateFuture.ExStateFuture<T> stateFuture,
                              final EventHandler<StateEvent> handler) {
            super(name, controllerSupplier, errorFunc, new StateInt.ExStateInt(), stateFuture, handler);
        }
        ...
        public CompletableFuture<T> export() {
            final StateInt.ExStateInt          exState       = (StateInt.ExStateInt) state;
            final StateFuture.ExStateFuture<T> exStateFuture = (StateFuture.ExStateFuture<T>) stateFuture;
        ...
        protected CompletableFuture<T> onExporting2Closing(Runnable runnable, EventHandler<StateEvent> handler) {
            StateFuture.ExStateFuture<T> exStateFuture = (StateFuture.ExStateFuture<T>) stateFuture;
            CompletableFuture<T>         future        = stateFuture.newCloseFuture();
            //触发控制器中断等待
        ...
        protected void onFailedOpen(final Throwable throwable, final EventHandler<StateEvent> handler, final CompletableFuture<T> future) {
            //状态从打开中回滚到导出成功
            ((StateInt.ExStateInt) state).translate(StateInt.ExStateInt.OPENING, StateInt.ExStateInt.EXPORTED);
            publish(StateEvent.FAIL_OPEN, throwable, handler);
            future.completeExceptionally(throwable);
        }
        ...
        protected CompletableFuture<T> checkExport() {
            StateInt.ExStateInt          exState       = (StateInt.ExStateInt) state;
            StateFuture.ExStateFuture<T> exStateFuture = (StateFuture.ExStateFuture) stateFuture;
